      ...
      <div style={{ padding: '14px 15px', borderRadius: 12,
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ fontFamily: 'var(--font)', fontSize: 6.5, fontWeight: 700,
              letterSpacing: '0.2em', color: `${CYAN}80` }}>{tr('OPERATOR', 'ОПЕРАТОР')}</p>
            <p style={{ fontFamily: 'var(--font)', fontSize: 16, fontWeight: 900, marginTop: 3,
          ...
          <div style={{ textAlign: 'right', flexShrink: 0 }}>
            ...
            <p style={{ fontFamily: 'var(--font)', fontSize: 6.5, letterSpacing: '0.18em',
        ...
        {GATES.map(g => {
          ...
          return (
            <div key={g.key} style={{ display: 'flex', alignItems: 'center', gap: 9,
              ...
              <span style={{ fontFamily: 'var(--font)', fontSize: 8, fontWeight: 900, width: 22,
              ...
              <span style={{ fontSize: 8, color: open ? CYAN : 'rgba(148,163,184,0.35)' }}>
                {open ? '✓' : '🔒'}
              </span>
            </div>
          )
...
function Cell({ value, label, color }: { value: string; label: string; color: string }) {
  return (
    <div style={{ flex: 1, padding: '10px 12px', borderRadius: 10, textAlign: 'center',
      ...
      <p style={{ fontFamily: 'var(--font)', fontSize: 17, fontWeight: 900, color, lineHeight: 1,
        textShadow: `0 0 10px ${color}50` }}>{value}</p>
      <p style={{ fontFamily: 'var(--font)', fontSize: 6, letterSpacing: '0.14em', color: DIM,
        marginTop: 5 }}>{label}</p>
    </div>
  )
}
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 4 }}>
        <span style={{ fontFamily: 'var(--font)', fontSize: 7.5, fontWeight: 800, letterSpacing: '0.14em',
          color: idle ? 'rgba(148,163,184,0.4)' : stat.color, width: 96, flexShrink: 0 }}>
          {tr(stat.label, stat.ru)}
        </span>
        <span style={{ fontFamily: 'var(--font)', fontSize: 10, fontWeight: 900,
          color: idle ? 'rgba(148,163,184,0.35)' : stat.color, width: 26, flexShrink: 0 }}>
          {idle ? '—' : stat.value}
        </span>
